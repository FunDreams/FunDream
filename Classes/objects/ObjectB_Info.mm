...
#import "ObjectB_Info.h"
#import "UniCell.h"
#import "Ob_IconDrag.h"
#import "Ob_Editor_Interface.h"
#import "Ob_Arrow.h"

@implementation ObjectB_Info
//------------------------------------------------------------------------------------------------------
...
- (void)Start{
    ...
    if(pString!=nil){
        iIndexValue = ((*pString->pValueCopy)+SIZE_INFO_STRUCT)[0];
        BYTE bType= [m_pObjMng->pStringContainer->ArrayPoints GetTypeAtIndex:iIndexValue];
    ...
    if(m_iLayer==layerInterfaceSpace6){
     //   m_bLookTouch=YES;
        ...
        else if(pMode!=0 && *pMode==0 && m_bStartMove==YES)
        {
            ...
            if(pObGroup!=nil)
            {
                for (ObjectB_Info *pObob in pObGroup->m_pChildrenbjectsArr)
                {
