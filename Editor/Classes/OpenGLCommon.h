...
static inline Vector3D Vector3DRotateZ2D(Vector3D vector, GLfloat angle)
{
	float angleTmp=angle*3.14f/180;
	
	float Sin=sin(angleTmp);
	float Cos=cos(angleTmp);

	Vector3D ret;
	
	ret.x=(vector.x*Cos - vector.y*Sin);
	ret.y=(vector.x*Sin + vector.y*Cos);	
	ret.z=0;
