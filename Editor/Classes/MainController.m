...
//------------------------------------------------------------------------------------------------------
#ifdef GAME_CENTER
- (void)SubScore:(NSString *)Category withScore:(float)fScore
{
    if(m_bAutorization==NO)return;
    
    GKScore *scoreReporter = [[[GKScore alloc] initWithCategory:Category] autorelease];
    
    int64_t score_tmp = (int64_t)fScore;
    scoreReporter.value = score_tmp;
...
- (void)ShowLider:(NSString *)Category
{
    if(m_bAutorization==NO)return;
    ...
    if (leaderboardController != nil)
    {
        ...
        leaderboardController.modalPresentationStyle = UIModalPresentationCurrentContext;
...
- (void)leaderboardViewControllerDidFinish:(GKLeaderboardViewController *)viewController
{
    ...
    OBJECT_PERFORM_SEL(@"TouchView", @"ZeroMemory");
    [self Pause:NO];
}
#endif
//------------------------------------------------------------------------------------------------------
#ifdef BANNER_IAD
#pragma mark iAd Delegats Methods
...
- (void)viewDidLoad
{
    ...
    [localPlayer authenticateWithCompletionHandler:^(NSError *error) {
        ...
        {
            NSLog(@"Autorization Good");
            m_bAutorization=YES;
        }
        else
        {
            NSLog(@"Autorization Fail");
            m_bAutorization=NO;
        }
    }];
#endif
...
-(void) createGLView
{
    ...
    if(previousOrientation==UIInterfaceOrientationLandscapeLeft || previousOrientation==UIInterfaceOrientationLandscapeRight)
	{
        ...
        if ([self isDeviceAniPad])
            adView.frame=CGRectMake(0, mainScreen.bounds.size.width, 320, 50);
        else adView.frame=CGRectMake(0, mainScreen.bounds.size.width, 320, 50);
    }
    else
    {
        ...
        if ([self isDeviceAniPad])
            adView.frame=CGRectMake(0, mainScreen.bounds.size.height, 320, 50);
        else adView.frame=CGRectMake(0, mainScreen.bounds.size.height, 320, 50);
    }
...
//------------------------------------------------------------------------------------------------------
-(UInt32) LoadSoundRes:(NSString *)NameSound WithLoop:(bool)loop
{    
	UInt32 iSoundID = -1;
	
    NSString *TmpStringName=[NameSound lastPathComponent];
	if((iSoundID=[[m_pSoundList objectForKey:TmpStringName] intValue])!=0)return iSoundID;
	
	NSString * pBundle=(NSString *)[NameSound UTF8String];
	...
	if(pBundle!=nil)
	{
		...
		[m_pSoundList setObject:soundId forKey:TmpStringName];
	}
...
//------------------------------------------------------------------------------------------------------
-(void) PlaySoundMatrix:(float)NameSound pitch:(float)fPitch volume:(float)fVolume{
    
    int iNameSound=(int)NameSound;
    
    SoundEngine_SetEffectPitch(iNameSound,fPitch);
    SoundEngine_SetEffectLevel(iNameSound,fVolume);
    SoundEngine_StartEffect(iNameSound);
}
//------------------------------------------------------------------------------------------------------
...
//        m_pObjMng->fAngleRotateOffset=180;
        ...
        [[UIDevice currentDevice] performSelector:NSSelectorFromString(@"setOrientation:") withObject:(id)UIInterfaceOrientationPortraitUpsideDown];
...
{    
	...
	//	glBindTexture(GL_TEXTURE_2D, texture[m_iCount]);F
...
//        height*=0.01;
        ...
        TextureContainer *TmpContainer=[[TextureContainer alloc] InitWithName:NameFile
                                        WithUint:m_iTextureId WithWidth:sourceW WithWidth:sourceH];
